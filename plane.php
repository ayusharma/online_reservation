...
if(isset($_POST['from']) && isset($_POST['to']) && isset($_POST['day']) && isset($_POST['date']) && isset($_POST['month']) && isset($_POST['year']) && isset($_POST['class']) ){ 
    ...
    if(!empty($from) && !empty($to) && !empty($date) && !empty($month) && !empty($year) && !empty($class)){
       ...
       $query ="SELECT * FROM `plane` WHERE `source` LIKE '%$from%' AND `destination` LIKE '%$to%' AND `day` LIKE '%$day%' AND `class` LIKE '%$class%' ";
...
<!DOCTYPE html>
<html lang="en">
<head><title>Flights From <?php echo $fm.' to '.$too; ?></title></head>
<style>
    body {
        margin:0;
        padding:0;
...
<body>
    ...
    <header><h1>Flight From <?php echo $fm.' to '.$too; ?></h1></header>
    <table>
        <tr>
            <th>Flight No.</th>
            <th>Flight Name</th>
...
<?php
             while($result = mysql_fetch_array($query_run,MYSQL_ASSOC)){
                echo '<tr>
            <td>'.$result["plane_no"].'</td>
            <td>'.$result["plane_name"].'</td>
            <td>'.$result["$from"].'</td>
            <td>'.$result["$to"].'</td>
            <td>'.$class.'</td>
            <td>'.$result["$class"].'</td>
            <td><a href="book_plane.php?plane_no='.$result["train_no"].'&plane_name='.$result["plane_name"].'&from='.$fm.'&to='.$too.'&date='.$date.'&month='.$month.'&year='.$year.'&class='.$class.'&arr='.$result["$from"].'&depart='.$result["$to"].'&fare='.$result["$class"].'">Book</a></td>
                </tr>';
             }
      ?>      
    </table>  
